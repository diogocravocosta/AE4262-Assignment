...
Two better choices exist:
  ...
  2. **Mixture fraction Z  [-]**
     For a *premixed* freely-propagating flame Z is *uniform* across the
     entire domain (Z = Z_inlet = const, set by φ).  It therefore carries
     no spatial information and is NOT a useful coordinate for plotting
     species profiles within a single premixed flame.
     (Z is meaningful as a coordinate for *diffusion* flames or for
     comparing flames at different φ in a mixture-fraction space, but
     not for the intra-flame profiles requested here.)

     ┌───────────────────────────────────────────────────────────────────┐
     │  NOTE: this is a variable you may want to change depending on     │
     │  your analysis goal.  If you later perform a counterflow or       │
     │  jet-flame simulation, switching to coord='Z' (mixture fraction)  │
     │  is straightforward – see the `COORD` constant below.             │
     └───────────────────────────────────────────────────────────────────┘

  3. **Raw grid x [m]** – simplest; use coord='x_raw' for quick checks.

Set the module-level constant `COORD` to 'x_shifted' (default), 'x_raw',
or 'Z' to switch behaviour globally.
...
#:   'Z'         – mixture fraction (NOT meaningful for premixed flames; here
#:                 for completeness only – will produce a flat line since Z
#:                 is uniform in a freely-propagating premixed flame)
COORD: str = "x_shifted"
...
def _make_coord(res: SimResult) -> Tuple[np.ndarray, str]:
    ...
    """
    if COORD == "x_shifted":
        return (res.grid - res.x_flame) * 1e3, "Distance from reaction zone  (x − x_flame)  [mm]"
    elif COORD == "x_raw":
        return res.grid * 1e3, "Axial position  x  [mm]"
    elif COORD == "Z":
        # In a premixed flame, Z is uniform = φ/(φ + stoich_air_fuel_ratio)
        # For H₂: stoichiometric air-fuel ratio (by mole) = 1/(0.5) * (1 + 3.76) = 9.52
        # Z_st for H₂ ≈ 0.0283  (Bilger definition)
        warnings.warn(
            "COORD='Z' is NOT physically meaningful for a premixed freely-propagating flame "
            "because Z is uniform (= const) everywhere in the domain. "
            "The plot will be a flat line. Switch to 'x_shifted' for spatial profiles.",
            UserWarning,
            stacklevel=3,
        )
        # Return the physical grid anyway so the plot at least renders
        return (res.grid - res.x_flame) * 1e3, "Mixture fraction Z  [-]  ← uniform in premixed flame; see docstring"
    else:
        raise ValueError(f"Unknown COORD value: '{COORD}'. Choose 'x_shifted', 'x_raw', or 'Z'.")
...
def plot_single_phi(res: SimResult, save: bool = True) -> "matplotlib.figure.Figure":  # type: ignore[name-defined]
    """
    One plot per φ: all species mole fractions and HRR on the left axis,
    temperature on a right axis (different scale).

    Left axis  : mole fractions X(species)  +  HRR normalised to [0,1]
                 (normalised HRR lets it sit on the same axis as mole fractions
                 without dominating the scale; a note in the legend explains this)
    Right axis : Temperature [K]

    OH is highlighted with a thicker line because it is the key species for
    the §4.3 comparison: its profile extends further downstream than the HRR,
    which is why OH-PLIF images are broader than Abel-inverted OH* images.
    """
    import matplotlib.pyplot as plt

    fig, ax_sp = plt.subplots(figsize=(9, 5))
    ax_T = ax_sp.twinx()   # right axis for temperature

    xc = res.coord_values

    # ── Species colours and line widths ──────────────────────────────────
    colours = {
        "OH":  "#2ca02c",   # green  – most important; highlight
        "H2O": "#1f77b4",   # blue
        "H2":  "#ff7f0e",   # orange
        "O2":  "#9467bd",   # purple
        "H":   "#17becf",   # cyan
    ...
    for sp, xsp in res.X.items():
        ...
        ax_sp.plot(xc, xsp,
                   ...
                   linewidth=lw_map.get(sp, 1.2),
                   label=f"X({sp})")

    # ── HRR normalised to [0,1] on the species axis ───────────────────────
    hrr_norm = res.hrr / res.hrr.max()
    ax_sp.plot(xc, hrr_norm, color="red", linestyle="--", linewidth=1.8,
               label="HRR (normalised)  ←  OH* proxy")

    # ── Temperature on right axis ─────────────────────────────────────────
    ax_T.plot(xc, res.T, color="black", linewidth=2.0, linestyle="-",
              label="T [K]")
    ax_T.set_ylabel("Temperature  [K]", fontsize=11)
    ax_T.tick_params(axis="y")

    # ── Vertical marker at reaction zone ──────────────────────────────────
    x0 = 0.0 if COORD == "x_shifted" else res.x_flame * 1e3
    ax_sp.axvline(x0, color="grey", linestyle=":", linewidth=1.0,
                  label="Reaction zone (peak HRR)")

    # ── Labels ────────────────────────────────────────────────────────────
    ax_sp.set_xlabel(res.coord_label, fontsize=11)
    ax_sp.set_ylabel("Mole fraction  [-]  /  Normalised HRR  [-]", fontsize=11)
    ax_sp.set_title(
    ...
    # ── Combined legend (species + T) ─────────────────────────────────────
    lines_sp, labs_sp = ax_sp.get_legend_handles_labels()
    lines_T,  labs_T  = ax_T.get_legend_handles_labels()
    ax_sp.legend(lines_sp + lines_T, labs_sp + labs_T,
...
def plot_phi_sweep(results: List[SimResult], save: bool = True) -> "matplotlib.figure.Figure":  # type: ignore[name-defined]
    ...
    for res in results:
        ...
        ax_hrr.plot(xc, res.hrr / 1e6,
                    color=colour, linewidth=1.5, label=f"φ={res.phi:.1f}")

    for ax, ylabel, title in [
        (ax_oh,  "Mole fraction X(OH)  [-]",   "OH mole fraction  (ground state → OH-PLIF signal)"),
        (ax_hrr, "Heat release rate  [MW/m³]", "Heat release rate  (reaction zone → OH* signal)"),
    ]:
