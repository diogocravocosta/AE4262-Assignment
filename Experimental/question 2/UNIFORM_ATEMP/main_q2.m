...
trunc_tip  = 0.10;   % fraction trimmed from the tip  end of below-tip rows for fitting
trunc_base = 0.10;   % fraction trimmed from the base end of below-tip rows for fitting
...
verbose_diag    = true;  % row-avg, intensity, and gradient figures
verbose_overlay = true;   % flame image with front overlay and fit
fit_lines_only  = false;   % overlay shows only fitted lines (no raw edges or boundaries)

results_H2 = flamefront_sf(H2_image_locs, ...
    1/px_mm_x_H2, 1/px_mm_y_H2, threshold, drop_frac, trunc_tip, trunc_base, ...
    data_H2(:,2)'*1000, ...
    verbose_diag, verbose_overlay, fit_lines_only);

results_CH4 = flamefront_sf(CH4_image_loc, ...
    1/px_mm_x_CH4, 1/px_mm_y_CH4, threshold, drop_frac, trunc_tip, trunc_base, ...
    data_CH4(:,2)'*1000, ...
    verbose_diag, verbose_overlay, fit_lines_only);
