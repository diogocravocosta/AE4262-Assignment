...
plt.title('a) Adiabatic Flame Temp vs Equivalence Ratio\n(Baseline: $T_{in}$=300K, P=1 bar)')
...
plt.title('b) Adiabatic Flame Temp vs Pressure\n(Baseline: $\phi$=0.8, $T_{in}$=300K)')
...
plt.title('c) Adiabatic Flame Temp vs Inlet Temperature\n(Baseline: $\phi$=0.8, P=1 bar)')
...
for fuel in fuels:
    ...
    for phi in phis:
        ...
        # Extract NO mole fraction (X) and convert to ppm (parts per million)
        # Note: gas['NO'].X returns an array of length 1, so we take the [0] index
        no_frac = float(gas['NO'].X[0])
        NO_ppm_d.append(no_frac * 1e6) 
        
    plt.plot(phis, NO_ppm_d, '-d', color=colors[fuel], label=labels[fuel], linewidth=2)

plt.title('d) Equilibrium NO Emissions vs Equivalence Ratio\n(Baseline: $T_{in}$=300K, P=1 bar)')
plt.xlabel('Equivalence Ratio, $\phi$')
plt.ylabel('NO Emissions (ppm)')
plt.yscale('log') # NO varies by orders of magnitude, so log scale is best here
